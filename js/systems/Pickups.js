    ...
    static spawn(scene, levelConfig) {
        return (levelConfig.pickups || [])
            .map(cfg => {
                ...
                if (cfg.type === 'invincible') return new InvinciblePickup(scene, cfg);
                return null;
            })
            .filter(Boolean);
    }
...
class InvinciblePickup {
    constructor(scene, cfg) {
        this.scene = scene;
        this.x = cfg.x;
        this.y = cfg.y ?? (scene.levelHeight - 68);
        this.duration = cfg.duration != null && !Number.isNaN(cfg.duration) ? Math.max(0, cfg.duration) : 3000;
        this.collected = false;
        const size = 24;

        this.sprite = scene.add.circle(this.x, this.y, size / 2, 0xffdd66, 0.9)
            .setStrokeStyle(2, 0xfff0aa, 0.95)
            .setDepth(12);
        this.label = scene.add.text(this.x, this.y, '🛡', {
            font: '13px Arial',
            color: '#ffffff'
    ...
    collect() {
        ...
        this.collected = true;
        player.grantInvincibility(this.duration);
        Effects.hitFlash(this.scene, this.x, this.y - 8);
        this._destroy();
    }
